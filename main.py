...
class SandBoxState(BaseState):
	def __init__(self, fsm):
		...
		if self.platform == "win32":
			...
			self.drives = [disk for disk in listdir(path.expandvars("/media/$USER"))]
			...
			if action == "Exit":
				...
				if self.platform == 'Windows':
					...
					if path.isabs('/'):
						...
						linux_action = path.join(path.expandvars("/media/$USER"), action)
						try:
							scandir(linux_action)
							self.fsm.ch_state(SandBoxState(self.fsm), {"curr_dir": linux_action})
						except:
							self.error_text = TextLine("Access denied", self.drive_font, (120, 50), font_colour=rgb.RED)
